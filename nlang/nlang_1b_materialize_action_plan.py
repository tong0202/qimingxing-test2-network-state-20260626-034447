...
def build_f3_state(
    ...
    state = {
        ...
        "mainline_contract": {
            ...
            "rule": "This F3 state was advanced by an NLANG-compiled action_plan materializer.",
        },
        ...
        "truth_boundary": "F3 state was advanced by NLANG-1B materialization; this is still external-runner execution over mutable remote anchors.",
        "state_hash": "",
    }
    return seal(state, "state_hash")


def append_proof_ledger(owner: str, repo: str, token: str, run_id: str, entry: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    write = put_json(owner, repo, token, PROOF_LEDGER_PATH, ledger, f"NLANG-1B proof ledger {run_id}")
    return ledger, write
...
def main() -> int:
    ...
    if not isinstance(remote_f3, dict):
        result = {"stage": "NLANG-1B-materialize", "ok": False, "error": "missing_remote_f3_state", "status": f3_response.get("status")}
    ...
    f2_capsule_write = put_json(args.owner, args.repo, token, "states/f2-scheduler-capsule.json", f2_capsule, f"NLANG-1B refresh F2 scheduler capsule {run_id}")
    ...
    f3_capsule_write = put_json(args.owner, args.repo, token, "states/f3-loop-capsule.json", f3_capsule, f"NLANG-1B refresh F3 capsule {run_id}")
    ...
    f2_state_write = put_json(args.owner, args.repo, token, F2_STATE_PATH, f2_state, f"NLANG-1B update F2 state {run_id}")
    f2_state_check = wait_for_hash(args.owner, args.repo, token, F2_STATE_PATH, "state_hash", f2_state["state_hash"])
    f2_entry = {
        "stage": "NLANG-1B-entry-over-f2-scheduler",
        "created_at": now(),
        "run_id": run_id,
    ...
    f3_state_write = put_json(args.owner, args.repo, token, F3_STATE_PATH, f3_state, f"NLANG-1B materialized F3 state {run_id}")
    f3_state_check = wait_for_hash(args.owner, args.repo, token, F3_STATE_PATH, "state_hash", f3_state["state_hash"])
    f3_entry = {
        "stage": "NLANG-1B-entry-over-f3-loop",
        "created_at": now(),
        "run_id": run_id,
        "owner": run_owner(args.mode),
    ...
    f3_result = {
        "run_id": run_id,
        "ok": all_ok,
        "evidence_level": "NLANG-1B-materialized-action-plan-v0",
        "selected_actions": [action],
        "window_count": f3_state["window_count"],
        "lifecycle_cycle_count": f3_state["lifecycle_cycle_count"],
        ...
        "truth_boundary": "NLANG-1B materializes a compiled action_plan with an external runner; it is not CPU-free execution.",
        "conclusion": "NLANG-1B materialized one low-risk NLANG-selected action into F2/F3 remote state.",
    }
    f3_last_run = build_f3_last_run(f3_result)
    f3_last_report = build_f3_last_report(f3_result)
    f3_last_run_write = put_json(args.owner, args.repo, token, F3_LAST_RUN_PATH, f3_last_run, f"NLANG-1B update F3 last run {run_id}")
    f3_last_report_write = put_json(args.owner, args.repo, token, F3_LAST_REPORT_PATH, f3_last_report, f"NLANG-1B update F3 last report {run_id}")

    proof_entry = {
        "stage": "NLANG-1B-proof-entry",
        "created_at": now(),
        "run_id": run_id,
    ...
    proof_ledger, proof_ledger_write = append_proof_ledger(args.owner, args.repo, token, run_id, proof_entry)
    proof_ledger_check = wait_for_hash(args.owner, args.repo, token, PROOF_LEDGER_PATH, "ledger_hash", proof_ledger["ledger_hash"])
    ...
    result = {
        "stage": "NLANG-1B-materialize-action-plan",
        "ok": core_ok,
        "run_id": run_id,
        "remote_f3_state_hash_before": remote_f3.get("state_hash"),
        "remote_f3_state_hash_ok": remote_hash_ok,
        ...
        "truth_boundary": "NLANG-1B proves compiled-plan materialization over remote anchors. It does not prove endpoint-free or CPU-free execution.",
    }
    out_path = Path(args.out)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    put_json(args.owner, args.repo, token, LAST_RUN_PATH, result, f"NLANG-1B last run {run_id}")
    print(json.dumps(result, ensure_ascii=False, indent=2))
