...
LAST_RUN_PATH = "states/nc-17-behavior-cycle/last-run.json"
LEDGER_PATH = "states/nc-17-behavior-cycle/ledger.json"
...
def github_request(method: str, path: str, token_value: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "qimingxing-nc17-behavior-cycle-worker",
        },
    )
...
def choose_behavior_op(summary: dict[str, Any]) -> tuple[str, str]:
    ...
    if summary.get("safety_risk_level") != "low" or not summary.get("safety_last_attested_at"):
        op = "safety_attest"
        reason = "safety slot is not freshly attested as low risk"
    elif summary.get("intent_focus") != TARGET_INTENT:
        op = "intent_focus"
        reason = "intent slot is not focused on the minimal behavior cycle"
    elif int(summary.get("control_tick") or 0) <= int(summary.get("memory_entry_count") or 0):
        op = "control_tick"
        reason = "control tick has not advanced beyond memory entry count"
    else:
        op = "memory_append"
        reason = "control is ahead, so append a new memory event"
...
def ensure_bank(bank: dict[str, Any] | None, run_id: str, active_state: dict[str, Any] | None) -> dict[str, Any]:
    if isinstance(bank, dict) and isinstance(bank.get("slots"), dict):
        return bank
    bank_obj = {
        "stage": "NC-17-bootstrap-register-bank",
        "schema_version": "QMX-NC-17-BANK-BOOTSTRAP-0.1",
        "run_id": run_id,
        "bank_seq": 0,
        "prev_bank_hash": "",
        ...
        "created_at": now_iso(),
        "updated_at": now_iso(),
        "truth_boundary": "NC-17 bootstrap bank is remote repository state, not CPU-free computation.",
        "bank_hash": "",
    }
    return seal(bank_obj, "bank_hash")
...
def apply_behavior_op(
    ...
    *,
    ...
    event = {
        ...
        "decision_source": "remote_register_bank_policy",
    ...
    if op == "memory_append":
        ...
        slots["control"]["mode"] = "behavior_cycle"
        ...
        slots["safety"]["last_attested_by"] = "nc17_behavior_cycle_worker"
    elif op == "bank_init":
        slots["control"]["mode"] = "observe"
        slots["control"]["last_op"] = op
    else:
        raise RuntimeError(f"unsupported behavior op: {op}")

    behavior = next_bank.get("behavior_cycle") if isinstance(next_bank.get("behavior_cycle"), dict) else {}
    ...
    next_bank["behavior_cycle"] = behavior
    next_bank["stage"] = "NC-17-behavior-cycle-register-bank"
    ...
    next_bank["remote_executor"] = {
        ...
        "workflow": "nc17-behavior-cycle.yml",
        "github_run_id": os.environ.get("GITHUB_RUN_ID", ""),
        "github_run_attempt": os.environ.get("GITHUB_RUN_ATTEMPT", ""),
        "event_name": os.environ.get("GITHUB_EVENT_NAME", ""),
        "cycle_index": cycle_index,
    }
    next_bank["truth_boundary"] = "NC-17 behavior cycle is remote repository state transition, not CPU-free or endpoint-free computation."
...
def append_ledger(owner: str, repo: str, token_value: str, entries_to_add: list[dict[str, Any]], run_id: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    ledger = seal(
        {
            "stage": "NC-17-behavior-cycle-ledger",
            "schema_version": "QMX-NC-17-LEDGER-0.1",
            "updated_at": now_iso(),
            "latest_run_id": run_id,
            "entry_count": len(merged),
            "entries": merged,
            "truth_boundary": "NC-17 ledger records remote register-bank policy decisions and writes.",
            "ledger_hash": "",
        },
        "ledger_hash",
    )
    write = put_json(owner, repo, LEDGER_PATH, token_value, ledger, f"NC-17 ledger {run_id}")
    return ledger, write
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    run_id = args.run_id or f"nc17-behavior-cycle-{os.environ.get('GITHUB_RUN_ID', 'local')}-attempt-{os.environ.get('GITHUB_RUN_ATTEMPT', '1')}"
    ...
    for cycle_index in range(args.cycles):
        ...
        write = put_json(owner, repo, BANK_PATH, token_value, next_bank, f"NC-17 behavior cycle seq {next_bank['bank_seq']} {run_id}")
        decision = {
            "cycle_index": cycle_index,
            "decision_source": "remote_register_bank_policy",
    ...
    last_run = seal(
        {
            "stage": "NC-17-behavior-cycle-last-run",
            "schema_version": "QMX-NC-17-LAST-RUN-0.1",
            "created_at": now_iso(),
            "run_id": run_id,
            "ok": ok,
            ...
            "client_payload_contract": {
                "local_supplied_ops": False,
                "local_supplied_policy": False,
                "local_supplied_cycles_only": True,
            },
            "decisions": decisions,
            "writes": writes,
            "ledger_hash": ledger.get("ledger_hash"),
            "ledger_write_ok": bool(ledger_write.get("ok")),
            "truth_boundary": "NC-17 proves a controlled remote behavior cycle from register-bank state, not CPU-free or endpoint-free network-native computation.",
            "last_run_hash": "",
        },
        "last_run_hash",
    )
    last_run_write = put_json(owner, repo, LAST_RUN_PATH, token_value, last_run, f"NC-17 last run {run_id}")
    result = {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="NC-17 remote behavior cycle worker")
